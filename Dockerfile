...
# is needed from the distribution. Add tesseract-ocr-<lang> packages for more
# languages and pass -lang accordingly.
FROM debian:bookworm-slim
RUN apt-get update \
 && apt-get install -y --no-install-recommends tesseract-ocr tesseract-ocr-eng ca-certificates curl \
...
HEALTHCHECK --interval=30s --timeout=5s --start-period=20s --retries=3 \
  CMD curl -fsS http://127.0.0.1:9090/api/info >/dev/null || exit 1

ENTRYPOINT ["pdf2word"]
CMD ["-addr", "0.0.0.0:9090", "-no-browser"]
