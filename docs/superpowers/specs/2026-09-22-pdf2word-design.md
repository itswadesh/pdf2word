...
## 17. Exact pagination (2026-09-23) — implemented

Goal: the Word file paginates like the PDF, line for line, on a machine that may not have the PDF's fonts.

- **Lines.** Every PDF line becomes a Word line inside its paragraph (`<w:br/>`); `Options.Reflow` joins them instead. Effective font size = Tf × text-matrix scale. Word gaps from advance boxes; PDFium-generated spaces ignored. Letter-spaced lines carry character spacing. Justified paragraphs narrower than the measure get a right indent.
- **Fonts.** Families that ship with Windows/Office keep their names; others are replaced by Times New Roman / Arial / Courier New by class (name hints, fixed-pitch flag; unknown → Times). `fontTable.xml` declares `altName`; `settings.xml` asks for compatibility mode 15; document defaults enable kerning and disable widow control.
- **Fitting.** For each kept line the natural width in the substitute face is measured from installed font files (`internal/fontmetrics`; Windows fonts or Liberation/Carlito/Caladea). If it exceeds the PDF line width × 0.994, character spacing is reduced (rounded to whole twips away from zero, counted per gap, up to 1 pt), else horizontal scale down to 75 %. Lines with tabs or pictures, or without metrics, are left alone.
- **Pages.** Margins: per side the smallest among pages within 12 pt of the median (covers ignored), plus 1.5 pt measure slack and 10 pt bottom slack. A page whose content lies outside the unslacked margins keeps its own setup and becomes its own section; positions are relative to the margins actually used. New pages start with `pageBreakBefore`; section properties sit in the last paragraph of the page; empty pages are one 1 pt paragraph. Pictures are scaled into the content box both ways.
- **Vertical model.** Baselines from glyph origins; leading = median baseline distance; a paragraph's Word box runs from first baseline − (leading − 0.22 em) to last baseline + 0.22 em; space before = gap to the previous box.
- **Pictures on OCR pages.** No words → picture kept; few words → `LooksLikePicture` (mid-tone share ≥ 15 %) decides cover vs short scan; ≥ 40 words → scan. Words inside a kept picture are dropped; a picture ≥ 80 % of the page drops all words.
